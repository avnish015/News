...
final class ArticleListServiceTests: XCTestCase {
    func testArticleListServiceSuccess() throws {
        let expectation = expectation(description: "ArticleListSuccess")
        let service = ArticleListService(apiClient: MockApiClient())
        
        var articleList: [ArticleDataModel] = []
        var theError: Error? = nil
        
        service.fetchArticleList(resource: MockNewsListResource(resourceName: .NewsList))
            .done { list in
                articleList = list
                expectation.fulfill()
            }.catch { error in
    ...
    func testArticleListServiceFailure() throws {
        let expectation = expectation(description: "ArticleListFailure")
        let service = ArticleListService(apiClient: MockApiClient())
        var theError: Error? = nil
        
        service.fetchArticleList(resource: MockNewsListResource(resourceName: .NewsListNoResponse))
            .done { list in
                expectation.fulfill()
            }.catch { error in
                theError = error
