...
final class ArticleListRepositoryTests: XCTestCase {
    ...
    func testArticleListUseCaseSuccess() throws {
        ...
        repository.fetchArticleList(resource: MockNewsListResource(resourceName: .NewsList))
            .done { list in
                articleList = list
                expectation.fulfill()
            }.catch { error in
    ...
    func testArticleListUseCaseFailure() throws {
        ...
        repository.fetchArticleList(resource: MockNewsListResource(resourceName: .NewsListNoResponse))
            .done { list in
                expectation.fulfill()
            }.catch { error in
                theError = error
