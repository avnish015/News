...
final class TestURLSessionAPIClient: XCTestCase {
    ...
    func testSuccess() throws {
        ...
        apiClient.callApi(responseType: ArticleResponse.self, resource: MockNewsListResource(resourceName: .NewsList))
            .map({$0.articles})
            .done { list in
                articleList = list
                expectation.fulfill()
    ...
    func testFailure() throws {
        ...
        apiClient.callApi(responseType: ArticleResponse.self, resource: MockNewsListResource(resourceName: .NewsListNoResponse))
            .done { list in
                expectation.fulfill()
            }.catch { error in
                theError = error
